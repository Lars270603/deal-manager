    ...
    >
      <div style={{
        ...
        <div
          {...attributes}
          {...listeners}
          style={{ cursor: 'grab', color: 'var(--text-muted)', flexShrink: 0, padding: '8px 2px', display: 'flex', alignItems: 'center' }}
        >
...
function ListingModal({ open, onClose, editing, onSave }) {
    ...
    if (editing) {
      ...
      supabase
        ...
        .then(({ data }) => {
          ...
          if (Array.isArray(plan) && plan.length > 0 && plan[0] && typeof plan[0] === 'object' && 'start_date' in plan[0]) {
            setRotationSlots(plan.map(entry => ({
              id: Math.random().toString(36).slice(2),
              variantTempId: entry.variant_id || null,
  ...
  const updateSlot = (slotId, field, value) => {
    setRotationSlots(prev => prev.map(s => s.id === slotId ? { ...s, [field]: value } : s))
  }
  ...
  const fullInputStyle = { ...inputStyle(), width: '100%', boxSizing: 'border-box' }
  ...
  return (
    ...
    >
      <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            ...
            {form.image_url && !imgError && (
              <img src={form.image_url} alt="Vorschau" onError={() => setImgError(true)}
        ...
        {/* Varianten (only for varianten type) */}
        {form.type === 'varianten' && (
          <div>
        ...
        <div style={{ background: 'var(--bg-overlay)', borderRadius: 10, padding: '16px 16px 12px', border: '1px solid var(--border-subtle)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <label style={{ ...labelStyle, marginBottom: 0 }}>
              {form.type === 'einzel' ? 'Aktive Zeiträume' : 'Rotationsplan'}
            </label>
            <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
              {rotationSlots.length} {rotationSlots.length === 1 ? 'Eintrag' : 'Einträge'}
            </span>
          </div>
            ...
            {rotationSlots.map(slot => (
              <div
                ...
                style={{
                  ...
                  borderRadius: 7,
                  border: `1px solid ${slot.start_date && slot.end_date ? 'rgba(34,197,94,0.2)' : 'var(--border-subtle)'}`,
                }}
              >
                {form.type === 'varianten' && (
                  <select
                    value={slot.variantTempId || ''}
                    onChange={e => updateSlot(slot.id, 'variantTempId', e.target.value || null)}
                    style={{
                      flex: 1, minWidth: 80,
                      padding: '5px 8px',
                      background: 'var(--bg-base)',
                  ...
                  >
                    ...
                    {namedVariants.map(v => (
                      <option key={v.tempId} value={v.tempId}>{v.name}</option>
                    ))}
                  </select>
                )}
                <input
                  ...
                  onChange={e => updateSlot(slot.id, 'start_date', e.target.value)}
                ...
                <input
                  ...
                  value={slot.end_date}
                  min={slot.start_date}
                  onChange={e => updateSlot(slot.id, 'end_date', e.target.value)}
                  style={inputStyle({ fontSize: 12 })}
                  title="Enddatum"
                />
                <button
...
// ─── Listing Card ───────────────────────────────────────────────────────────
function ListingCard({ listing, variants, today, onEdit, onDelete, dealStatus }) {
  const [hovered, setHovered] = useState(false)
  const [copiedAsin, setCopiedAsin] = useState(false)
  const brand = BRANDS[listing.brand]
  const active = getActiveVariant(listing, variants, today)
          ...
          {hovered && (
            ...
            >
              <button
                ...
                style={{ background: 'rgba(8,8,14,0.8)', border: '1px solid var(--border-default)', borderRadius: 6, padding: 5, cursor: 'pointer', color: 'var(--text-primary)', display: 'flex', alignItems: 'center' }}
              >
                <Edit2 size={13} />
              </button>
              <button
                ...
                style={{ background: 'rgba(8,8,14,0.8)', border: '1px solid var(--border-default)', borderRadius: 6, padding: 5, cursor: 'pointer', color: '#ef4444', display: 'flex', alignItems: 'center' }}
              >
                <Trash2 size={13} />
              </button>
            </motion.div>
      ...
      <div style={{ padding: '12px 14px' }}>
        ...
        {active && active !== 'pause' && (
          <div style={{
            ...
            ✓ Heute: {typeof active === 'object' ? active.name : 'Aktiv'}
          </div>
        )}
        ...
        {asin && (
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, color: 'var(--text-muted)' }}
          >
            <code style={{ fontSize: 11, fontFamily: 'monospace', color: 'var(--text-secondary)' }}>{asin}</code>
            {copiedAsin ? <Check size={11} style={{ color: 'var(--green)' }} /> : <Copy size={11} />}
          </button>
        )}
      </div>
    </motion.div>
...
export default function Listings() {
  ...
  const { dealStatus } = useDealStatus(today)
  ...
  const handleSave = async (form, formVariants, rotationSlots, editing) => {
    const validSlots = rotationSlots.filter(s => s.start_date && s.end_date)

    const buildRotationPlan = (tempToReal) => {
      if (form.type === 'varianten') {
        return validSlots.map(s => ({
          variant_id: s.variantTempId ? (tempToReal[s.variantTempId] || s.variantTempId) : null,
          start_date: s.start_date,
          end_date: s.end_date,
        }))
      }
      return validSlots.map(s => ({ start_date: s.start_date, end_date: s.end_date }))
    }
    ...
    if (editing) {
      if (form.type === 'varianten') {
        ...
        await updateListing(editing.id, { ...basePayload, rotation_plan: buildRotationPlan(tempToReal) })
      } else {
        await updateListing(editing.id, { ...basePayload, rotation_plan: buildRotationPlan({}) })
      }
      toast.success('Listing gespeichert')
    } else {
      const newListing = await createListing({ ...basePayload, rotation_plan: [] })
      if (form.type === 'varianten' && formVariants.length > 0) {
        const tempToReal = await saveVariantsForListing(newListing.id, formVariants)
        await updateListing(newListing.id, { rotation_plan: buildRotationPlan(tempToReal) })
      } else if (form.type === 'einzel' && validSlots.length > 0) {
        await updateListing(newListing.id, { rotation_plan: buildRotationPlan({}) })
      }
          ...
          {[null, 'varianten', 'einzel'].map(t => (
            <button
              ...
              style={{
                ...
                background: typeFilter === t ? 'var(--accent-muted)' : 'var(--bg-elevated)',
                color: typeFilter === t ? 'var(--accent)' : 'var(--text-secondary)',
                cursor: 'pointer', fontSize: 12, fontFamily: 'inherit', fontWeight: 600,
                transition: 'all 0.15s',
              }}
            >
              {t === null ? 'Alle Typen' : t === 'varianten' ? 'Varianten' : 'Einzel'}
            </button>
          ))}
        </div>
      </motion.div>
