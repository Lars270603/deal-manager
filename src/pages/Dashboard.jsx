    ...
    <div style={{
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{
            fontSize: 11, color: '#6B6B8A', marginTop: 6,
...
function WeekRow({ listing, active }) {
  ...
  return (
    <div style={{
      ...
      <span style={{
        flex: 1, fontSize: 13, color: '#D0D0E0',
    ...
    <div style={{
      ...
      {loading ? <SkeletonLoader count={4} height={22} gap={4} /> : (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr repeat(4, 46px)', gap: 3, marginBottom: 5 }}>
            ...
            {weeks.map(w => (
              <div key={`h-${w.kw}`} style={{
                ...
                color: w.kw === kw && w.year === year ? '#C41E3A' : '#6B6B8A',
                ...
                {weeks.map(w => {
                  ...
                  return (
                    <div key={w.kw} style={{
                      ...
                      background: isActive ? `${brand.color}26` : 'rgba(255,255,255,0.03)',
          ...
          {relevant.length === 0 && (
            <p style={{ fontSize: 12, color: '#6B6B8A', textAlign: 'center', padding: '10px 0' }}>
              Keine aktiven Deals
            </p>
          )}
        </>
    ...
    <div style={{
      ...
      {loading ? <SkeletonLoader count={3} height={26} gap={4} /> : (
        <div>
            ...
            return (
              <div key={listing.id} style={{
                ...
                <div style={{
                  ...
                  border: done ? 'none' : '1.5px solid #3E3E52',
                ...
                {asin && (
                  <code style={{ fontSize: 10, fontFamily: 'monospace', color: '#484860', flexShrink: 0 }}>
                    {asin}
                  </code>
                )}
              </div>
          ...
          {total === 0 && (
            <p style={{ fontSize: 12, color: '#6B6B8A', padding: '6px 0' }}>Keine aktiven Deals</p>
          )}
        </div>
      )}
...
export default function Dashboard() {
  ...
  const stats = [
    { label: 'Aktive Listings',   value: listings.length, color: '#FFFFFF', borderColor: '#C41E3A', loading: listingsLoading },
    { label: 'Deals diese Woche', value: thisWeek.length, color: '#22C55E', borderColor: '#22C55E', loading: listingsLoading },
    { label: 'Einzureichen',      value: stillToSend,     color: '#F59E0B', borderColor: '#F59E0B', loading: statusLoading },
    { label: 'William offen',     value: pendingWilliam,  color: '#3B82F6', borderColor: '#3B82F6', loading: statusLoading },
  ]
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 26 }}>
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.38 }}>
          <h1 style={{ fontFamily: 'Georgia, serif', fontWeight: 700, fontSize: 32, margin: 0, color: '#FFFFFF', lineHeight: 1.15 }}>
            {getGreeting()}
          </h1>
          <p style={{ fontSize: 13, color: '#6B6B8A', margin: '8px 0 0', lineHeight: 1.6 }}>
        ...
        >
          <img src={albatrosLogo} alt="" style={{ width: 100, height: 100, objectFit: 'contain', opacity: 0.12 }} />
        </motion.div>
      </div>
      ...
      >
        ...
        <div style={{
          ...
          {listingsLoading ? (
            ...
            <p style={{ color: '#6B6B8A', fontSize: 13 }}>Keine aktiven Deals diese Woche</p>
          ) : (
